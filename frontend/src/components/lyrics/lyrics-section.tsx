...
export function LyricsSection() {
  const { trackId, trackName, artistName, selectedLines, setSelectedLines } =
    useCardParams();
    ...
    <Card>
        ...
        <CardTitle className="text-lg flex items-center justify-between">
          ...
          {selectedLines.length > 0 && (
            <span className="text-sm font-normal text-muted-foreground">
              ...
              sélectionnée{selectedLines.length > 1 ? "s" : ""}
            </span>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <p className="text-xs text-muted-foreground mb-4">
          Clique sur une ligne pour commencer la sélection, puis sur une autre
          pour terminer (max {MAX_SELECTED_LINES} lignes)
        </p>
